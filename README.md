...
ゲームは複数のカテゴリに分かれた計 19 ステージで構成されています：

| カテゴリ | ステージ数 | 特徴                             |
| -------- | ---------- | -------------------------------- |
| Intro    | 3          | チュートリアル（基本操作の習得） |
| Level 1  | 3          | 基本問題（にれつ〜よんれつ）     |
| Level 2  | 3          | 縦長問題（たかく）               |
| Level 3  | 3          | 横長問題（ひろく）               |
| Level 4  | 3          | 階段型問題（かいだん）           |
| Level 5  | 3          | 積み重ね問題（かさね）           |
| Outro    | 1          | 総合問題（連勝で難易度上昇）     |

各カテゴリの 3 つ目は「Ex（エキストラ）」モードで、空き列なしの高難度ステージです。
...
- **ビルドツール**: esbuild（本番ビルドのみ）
- **開発時**: ネイティブ ESM（ビルド不要）
- **PWA**: Service Worker によるオフライン対応
- **対応ブラウザ**: CSS Nesting（`selector(&)`）をサポートするモダンブラウザ
...
│   └── x-*.m.html            # 汎用コンポーネント
├── lib/                  # ユーティリティ
│   ├── audio.js              # オーディオ管理（BGM・SE）
│   ├── state.js              # 状態管理（localStorage/sessionStorage連携）
│   ├── route-state.js        # ルーティング
│   ├── random.js             # 乱数生成・シード管理
│   ├── user-activation.js    # ユーザーアクティベーション管理
│   ├── asset.js              # アセット読み込み（本番用）
│   ├── asset.dev.js          # アセット読み込み（開発用）
│   └── utils.js              # 汎用ユーティリティ
├── rule/                 # ゲームロジック
│   ├── board-utils.js        # ボード操作・判定ロジック
│   ├── level.js              # レベル定義（19ステージ）
│   └── level-utils.js        # レベル生成・シャッフル処理
├── assets/               # アセット
│   ├── bgm/                  # BGM音源
│   ├── se/                   # 効果音
│   └── images/               # 画像リソース
...
- ステージクリア状況の自動保存（localStorage）
- アンドゥ機能（手戻り）
- 連勝システム（Outro ステージで難易度上昇）
- ヘルプ機能（操作説明・ルール解説）
